...
#from tickers import*
tickers_list = ['AAPL','AFRM','CRWD', 'DDOG', 'DOCN', 'DOCS','ENPH', 
'GOOGL','MQ','MTTR','NET','NVDA','OKTA','PYPL','QCOM', 'RBLX','TTD','TSLA','V']
#
header   = ['Ticker', 'Close','Adj Close','Max','Min','Volume', 'Market Cap','GM%', 
          'max drop','YTD', '1mo', '1wk', '1d']
title = "Watchlist"
...
for i in range(len(tickers_list)):
  ...
  #print(data['close'].iloc[-180:].min(),tickers_list[i])
  ...
  sorted_df = df.sort_values(by=['new'], ascending=False)
  print(sorted_df)

  pl = si.get_income_statement(tickers_list[i])
  if len(pl) != 0:  
    grossprofit = pl.iloc[6][0] #gross profit
    sales = pl.iloc[15][0] #sales
    
    if grossprofit != 0 and sales > 0:
      print(tickers_list[i])
      gm = grossprofit / sales 
      df.loc[tickers_list[i],'GM%']='{:.2%}'.format(gm)
...
fig = go.Figure(data=[go.Table(
  ...
  cells=dict(values=[sorted_df['Ticker'], sorted_df['Close'], sorted_df['Adj Close'], 
             ...
             sorted_df['GM%'], sorted_df['max drop'], sorted_df['YTD'], sorted_df['1mo'],
