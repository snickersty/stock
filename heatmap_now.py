...
yesterday2 = (datetime.datetime.today() - datetime.timedelta(days =3)).strftime('%Y-%m-%d')
max_per = 3 # maxのパーセンテージ (for coloring level)
## 1week ##### #diff_day  = 5 #max_per  = 3 # maxのパーセンテージ
## 1month ##### #diff_day  = 22 #max_per  = 10 # maxのパーセンテージ
# print(yesterday)
# print(si.get_data('AFRM')['close'])
# data = si.get_live_price('AFRM')
# data2 = si.get_data('AFRM')['close']
# data_pct=(data/data2[-1]-1)
# print(data,'/r',data2[-1],'/r',data_pct)
...
for name in categ.keys():
 ...
 for ticker in categ[name].keys():
    if ticker == '^N225' or ticker == 'BTC-USD':
        #print(ticker)
        data = si.get_live_price(ticker)
        data2 = si.get_data(ticker,end_date=yesterday)['close']
        data_pct=(data/data2[-1]-1)
        #print('check',ticker, data,data2[-1],data_pct,'/r')

        stock='${:,.2f}'.format(data)
        pct=data_pct
        chgpct='{:.2%}'.format(pct)
        ...
        #print('else',ticker)
        data = si.get_live_price(ticker)
        data2 = si.get_data(ticker,end_date=today)['close']
        data_pct=(data/data2[-1]-1)
...
draw.text((x_width*x_size-250, 10),datetime.datetime.now(pytz.timezone('America/New_York')).strftime('1day variance '+"%Y/%m/%d %H:%M"),'black',font=font_cate)

if not os.path.isdir('/'): os.makedirs('/')
im.save('now.png')
IPython.display.Image('now.png')
